...
const sharedLinkDynamicImageHTML = (questStartData) => {
  ...
  >
<div
          class="w-full h-full border-2 border-[#77797B] bg-[#1B1F23]"
        >
        <div
        class="border-[#389CE3] border-b-[#389CE3] bg-[#389CE3] border-b-[1.85px] p-4"
      >
...
<path id="Vector" d="M8.15784 16.6458C3.7029 16.6458 0.0791016 13.022 0.0791016 8.56702C0.0791016 4.11208 3.7029 0.488281 8.15784 0.488281C12.6128 0.488281 16.2366 4.11208 16.2366 8.56702C16.2366 13.022 12.6121 16.6458 8.15784 16.6458ZM8.15784 1.78088C4.41577 1.78088 1.3717 4.82495 1.3717 8.56702C1.3717 12.3091 4.41577 15.3532 8.15784 15.3532C11.8999 15.3532 14.944 12.3091 14.944 8.56702C14.944 4.82495 11.8993 1.78088 8.15784 1.78088ZM8.10614 9.16162H3.9569C3.78549 9.16162 3.6211 9.09352 3.49989 8.97232C3.37869 8.85112 3.3106 8.68673 3.3106 8.51532C3.3106 8.34391 3.37869 8.17952 3.49989 8.05831C3.6211 7.93711 3.78549 7.86902 3.9569 7.86902H7.45984V3.07348C7.45984 2.90207 7.52793 2.73768 7.64913 2.61648C7.77034 2.49527 7.93473 2.42718 8.10614 2.42718C8.27755 2.42718 8.44194 2.49527 8.56314 2.61648C8.68435 2.73768 8.75244 2.90207 8.75244 3.07348V8.51532C8.75244 8.68673 8.68435 8.85112 8.56314 8.97232C8.44194 9.09352 8.27755 9.16162 8.10614 9.16162Z" fill="#fff"/>
       ...
       ${calculateTimeAgo(questStartData?.createdAt)}
         ...
         <div class="p-5 h-[calc(100%-99.16px)] flex items-center justify-center">
            <h4
              id="dynamicText"
              class="font-semibold text-[#DCDDDD] text-[22px] leading-normal"
            >
    ...
    <script>
      const adjustFontSize = () => {
        ...
        // Define base font size when there are 350 characters
        const baseFontSize = 22;

        // Define minimum and maximum font sizes
        const minFontSize = 18; // Increased to avoid it being too small
        const maxFontSize = 25; // Lowered from 40px to prevent it from getting too large

        // Calculate the ratio of the current text length to the base length (350 characters)
        const lengthFactor = textLength > 0 ? 350 / textLength : 1;

        // Adjust the font size based on the length factor, ensuring it's proportional
        let newFontSize = baseFontSize * lengthFactor;

        // Further scale the font size based on the available container height
        const heightFactor = containerHeight / 250; // Assuming 250px is a good base height for 22px font

        newFontSize = newFontSize * heightFactor;

        // Clamp the new font size between the min and max font sizes
